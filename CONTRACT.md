...
1. `git clone --depth 1` this repo, then copies `<harness>/.` into `/root/workspace`
   (so files like `<harness>/AGENTS.md` become `/root/workspace/AGENTS.md`).
...
3. Loops: runs `launch.sh` from `/opt/tribes` (which `exec`s the harness), and drops
   ...
   scripts' location changed — `/root/workspace` is left holding just the harness's real
...
CFG=/root/workspace/.<harness>/config.json
