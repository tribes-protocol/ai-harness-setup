...
# previous boot's token), so refresh and re-point the on-disk api_key every launch.
# Match the api_key field value so the swap works for any prior token (a placeholder has no
